...
class Book_DataFrame():
    ...
    def Book_list_all(self) :
        ...
        for i in range(len(self.book_data['BOOK_ISBN'])):
            intext.append([set_str(self.book_data.iloc[i]['BOOK_TITLE']) + set_str(self.book_data.iloc[i]['BOOK_AUTHOR'], 24) + set_str(self.book_data.iloc[i]['BOOK_PUB'], 15) +  set_str(str(self.book_data.iloc[i]['BOOK_ISBN']), 13)])
            # intext.append([self.book_data.iloc[i]])
        return intext
    ...
    def Book_in(self, in_data) :
        # isbn=in_data[3] # title=in_data[0] # author=in_data[1] # pub=in_data[2] # price=in_data[4] # link=in_data[5] # description=in_data[6]
        new_data = pd.DataFrame([{'BOOK_ISBN' : in_data[3], 'BOOK_TITLE' : in_data[0], 'BOOK_AUTHOR' : in_data[1], 'BOOK_PUB' : in_data[2], 'BOOK_PRICE' : in_data[4], 'BOOK_LINK' : in_data[5], 'BOOK_DESCRIPTION' : in_data[6]}])
        
        if in_data[3] in self.book_data['BOOK_ISBN'] : # 이미 중복확인을 했음에도 중복 ISBN으로 등록했을 경우
...
# 한글, 영어 자리수 맞추기 함수
def set_str(input_s="", max_size=22, fill_char='  '):
    l = 0
    for c in input_s:
...
# print(set_str("abnacada"))
# print(set_str('한글한글한글한글'))
